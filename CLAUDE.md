...
- **Verify a frontend change renders (local-dev workaround below) BEFORE running `wrangler deploy`, not after — including when you're just picking up someone else's already-written, uncommitted change and deploying it "as-is".** `npm run build` succeeding proves nothing about runtime correctness (see the TDZ gotcha below). On 2026-07-19 a pending dashboard rebuild was committed and deployed straight from a stale `dist/` without a render check first — it had a `ReferenceError` that white-screened the entire live app, not just the dashboard, until a second session caught it via the local-dev workaround and shipped a fix. The build/deploy commands above are necessary but not sufficient; treat them as the last step after rendering, not a substitute for it.
...
- **R2 keys are stored with the LITERAL filename** (`${userId}/${propertyId}/${fileKey}/${token}/${file.name}` — spaces, `#`, `%` and all). But `new URL(request.url).pathname` arrives percent-encoded, so `GET /api/documents/*` must `decodeURIComponent` the sliced key before `CRM_DOCS.get`, or any file whose name contains a space (etc.) 404s on retrieval — View, inline open, and Re-parse all break for it. Fixed 2026-07-12. If you ever store keys differently, keep upload-write and GET-read symmetric.
...
- **`wrangler dev --local` can't exercise AI/Vectorize routes (2026-07-13).** In `--local` mode the Worker boots and KV/D1/R2/Assets bindings resolve, but `env.AI` and `env.VECTORIZE` report **"not supported"** — so any route that calls `env.AI` (all AI-insight routes, `env.AI.toMarkdown` doc extraction, `/api/ai/parse-rightmove`) or Vectorize (semantic search) throws locally. Non-`--local` `wrangler dev` fails on TLS in this environment. Net: verify AI/Vectorize routes against a **deployed** Worker (browser QA on the live URL) or via `npm test` + `wrangler deploy --dry-run`; there is no working local path for them.

## Frontend gotchas

- **`src/App.jsx` imports the lucide-react `Map` icon by name (line ~10), which shadows the global `Map` constructor for the ENTIRE file.** `new Map(...)` anywhere in App.jsx silently resolves to the icon component instead of the built-in class and throws `TypeError: Map is not a constructor` at render time — a build-time-invisible runtime crash with no error boundary, so it white-screens the whole app. **Never use `new Map()` in App.jsx** — use a plain object keyed by id instead (`const x = {}; arr.forEach(i => x[i.key] = ...)`). This is why no other code in the file uses `new Map()`. Same risk applies to any other lucide icon that collides with a JS/DOM global (audit the import list at the top of the file before reaching for a built-in by name). Bit us 2026-07-17: a triage-tab phase used `new Map()` for a lookup table, deployed clean (build/tests can't catch this), and white-screened the Auction Triage tab for ~3 hours across 5 stacked phases before being caught, because live-browser verification wasn't possible that session (see below) and nobody opened the tab.
- **Local dev-server workaround for "can't verify because the Browser pane is stuck on login."** The pane's session is isolated from the user's real browser and Claude cannot enter credentials — but auth in this app is entirely client-side gated (`Root` in `src/main.jsx` just checks `!!localStorage.getItem('crm_session')`). To smoke-test a UI change without a real login: start the local Vite dev server (`.claude/launch.json` config `property-crm-vite`, port 5173, or run `node node_modules/vite/bin/vite.js --port <N> --strictPort` directly if `preview_start` doesn't come up — it can take ~10-15s and prints "Establishing remote connection..." for the AI/Vectorize remote bindings first), then in the browser console: `localStorage.setItem('crm_session','dev-debug-token'); localStorage.setItem('crm_user', JSON.stringify({id:1,name:'Debug',email:'debug@local',role:'Admin'})); location.reload();` (role must be `'Admin'` or the entire nav sidebar renders empty — `can(tab)` in the sidebar checks `user.role`). This alone isn't enough: the app's mount effect (`App.jsx` ~2715, fetches `/api/crm-data`) gets a 401 against the bogus token and auto-clears `localStorage` + hard-reloads back to the login screen. Temporarily patch that one 401 branch to `setDataLoaded(true); return null;` instead of clearing storage/reloading, do the test, then **revert it before committing** — never ship the bypass. Attach `window.addEventListener('error', ...)` before clicking into the tab under test to capture the real stack trace (`read_console_messages` only surfaces React's generic "error occurred in <App>" warning, not the underlying `TypeError`). This is dev-only debugging against no real data/credentials, not a security bypass of the deployed app.
- **A `useEffect`/hook that references a `const [x, setX] = useState(...)` declared *later* in the same component throws `ReferenceError: Cannot access 'x' before initialization` on every render — a temporal-dead-zone crash, not a stale-closure bug.** Like the `new Map()` issue above, this is invisible to `npm run build` (it's a runtime error, not a syntax error) and white-screens the whole app with no error boundary, so a broken deploy looks identical to a browser cache/stale-build issue from the outside. Bit us 2026-07-19: the dashboard rebuild added an auto-briefing `useEffect` with `[dataLoaded]` in its dependency array ~600 lines above the `dataLoaded` state declaration (which lived down in the "KV persistence" block) — deployed clean, broke the entire app, not just the dashboard. Caught via the local-dev-server workaround below + `window.addEventListener('error')`; `read_console_messages` alone only shows React's generic "error occurred in <App>" boundary warning, not the underlying `ReferenceError`, so you have to check the dev-server terminal output (or inject an error listener before the crash) to see the real stack trace. When adding a hook near the top of a large component that depends on state declared further down, either move the state's declaration up or reorder the hook down — don't assume dependency-array references are lazily resolved.
- **Sidebar nav buttons are icon-only when collapsed** — no accessible `title`/`aria-label`, so `read_page`/`find` won't locate them by name. Query `document.querySelector('nav').querySelectorAll('button')` and index into the list (order matches `ALL_TABS` in App.jsx, filtered by `can(tab)`) instead.
